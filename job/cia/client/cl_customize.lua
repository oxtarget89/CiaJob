...
RegisterNetEvent('rom_cia:notify', function(title, desc, style)
    lib.notify({
        title = title,
        description = desc,
        duration = 3500,
...
--Send to jail
RegisterNetEvent('rom_cia:sendToJail', function(target, time)
...
impoundSuccessful = function(vehicle)
    ...
    if not DoesEntityExist(vehicle) then
        TriggerEvent('rom_cia:notify', Strings.success, Strings.car_impounded_desc, 'success')
    end
end
...
-- Customize target
AddEventHandler('rom_cia:addTarget', function(d)
...
AddEventHandler('rom_cia:changeClothes', function(data) 
...
-- Billing event
AddEventHandler('rom_cia:finePlayer', function()
    ...
    if player == -1 or dist > 3.5 then
        ...
        local jobLabel = lib.callback.await('rom_cia:getJobLabel', 100, hasJob)
        local targetId = GetPlayerServerId(player)
        local input = lib.inputDialog('Bill Patient', {'Amount'})
        if not input then return end
        local amount = math.floor(tonumber(input[1]))
        if amount < 1 then
            TriggerEvent('rom_cia:notify', Strings.invalid_entry, Strings.invalid_entry_desc, 'error')
...
openJobMenu = function()
    ...
    Options[#Options + 1] = {
        ...
        arrow = false,
        event = 'rom_cia:outVehiclePlayer',
    }
    Options[#Options + 1] = {
