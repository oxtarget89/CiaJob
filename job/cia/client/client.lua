...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    ...
    for i=1, #Config.ciaJobs do
        if Config.ciaJobs[i] == ESX.GetPlayerData().job.name then
...
RegisterNetEvent('esx:setJob', function(job)
    ...
    for i=1, #Config.ciaJobs do
        if Config.ciaJobs[i] == job.name then
            hasJob = job.name
            break
        else
            hasJob = nil
...
RegisterNetEvent('rom_cia:tackled', function(targetId)
    getTackled(targetId)
end)

RegisterNetEvent('rom_cia:tackle', function()
    tacklePlayer()
end)
...
AddEventHandler('rom_cia:searchPlayer', function()
    if not hasJob then return end
    local player, dist = ESX.Game.GetClosestPlayer()
    if player == -1 or dist > 2.0 then
        TriggerEvent('rom_cia:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
    else
        searchPlayer(player)
    end
end)

AddEventHandler('rom_cia:jailPlayer', function()
    if not hasJob then return end
    local player, dist = ESX.Game.GetClosestPlayer()
    if player == -1 or dist > 2.0 then
        TriggerEvent('rom_cia:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
        ...
        if quantity < 1 then
            TriggerEvent('rom_cia:notify', Strings.invalid_amount, Strings.invalid_amount_desc, 'error')
        else
            TriggerEvent('rom_cia:sendToJail', GetPlayerServerId(player), quantity)
        end
    end
end)

AddEventHandler('rom_cia:spawnVehicle', function(data)
    inMenu = false
...
AddEventHandler('rom_cia:handcuffPlayer', function()
    if not hasJob then return end
    local player, dist = ESX.Game.GetClosestPlayer()
    if player == -1 or dist > 2.0 then
        TriggerEvent('rom_cia:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
...
RegisterNetEvent('rom_cia:arrested', function(pdId)
    isBusy = true
    local escaped
...
RegisterNetEvent('rom_cia:arrest', function()
    isBusy = true
    lib.requestAnimDict('mp_arrest_paired', 100)
...
RegisterNetEvent('rom_cia:uncuffAnim', function(target)
...
RegisterNetEvent('rom_cia:uncuff', function()
    uncuffed()
end)

RegisterNetEvent('rom_cia:stopEscorting', function()
    if not escorting.active then return end
    escorting.active = nil
    escorting.target = nil
end)

AddEventHandler('rom_cia:escortPlayer', function()
    if not hasJob then return end
    local player, dist = ESX.Game.GetClosestPlayer()
    if player == -1 or dist > 2.0 then
        TriggerEvent('rom_cia:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
...
AddEventHandler('rom_cia:lockpickVehicle', function()
    local vehicle = ESX.Game.GetClosestVehicle()
    if not DoesEntityExist(vehicle) then
        TriggerEvent('rom_cia:notify', Strings.vehicle_not_found, Strings.vehicle_not_found_desc, 'error')
    else
        ...
        if dist < 2.5 then
            ...
            TriggerEvent('rom_cia:notify', Strings.too_far, Strings.too_far_desc, 'error')
        end
    end
end)

AddEventHandler('rom_cia:impoundVehicle', function()
    local vehicle = ESX.Game.GetClosestVehicle()
    if not DoesEntityExist(vehicle) then
        TriggerEvent('rom_cia:notify', Strings.vehicle_not_found, Strings.vehicle_not_found_desc, 'error')
    else
...
AddEventHandler('rom_cia:vehicleInfo', function()
    local vehicle = ESX.Game.GetClosestVehicle()
    if not DoesEntityExist(vehicle) then
        TriggerEvent('rom_cia:notify', Strings.vehicle_not_found, Strings.vehicle_not_found_desc, 'error')
    else
...
AddEventHandler('rom_cia:openBossMenu', function()
    if not hasJob then return end
...
RegisterNetEvent('rom_cia:escortedPlayer', function(pdId)
    if isCuffed then
...
RegisterNetEvent('rom_cia:setEscort', function(targetId)
    if not hasJob then return end
    escorting.active = not escorting.active
    escorting.target = targetId
end)

RegisterNetEvent('rom_cia:putInVehicle', function()
    if isCuffed then
...
RegisterNetEvent('rom_cia:takeFromVehicle', function()
...
AddEventHandler('rom_cia:inVehiclePlayer', function()
    if not hasJob then return end
    local player, dist = ESX.Game.GetClosestPlayer()
    if player == -1 or dist > 4.0 then
        TriggerEvent('rom_cia:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
    else
        TriggerServerEvent('rom_cia:inVehiclePlayer', GetPlayerServerId(player))
    end
end)

AddEventHandler('rom_cia:outVehiclePlayer', function()
    if not hasJob then return end
    local player, dist = ESX.Game.GetClosestPlayer()
    if player == -1 or dist > 4.0 then
        TriggerEvent('rom_cia:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
    else
        TriggerServerEvent('rom_cia:outVehiclePlayer', GetPlayerServerId(player))
    end
end)

AddEventHandler('rom_cia:vehicleInteractions', function()
    vehicleInteractionMenu(vehicle)
end)

AddEventHandler('rom_cia:placeObjects', function()
    placeObjectsMenu(vehicle)
end)

AddEventHandler('rom_cia:spawnProp', function(index)
...
AddEventHandler('rom_cia:licenseMenu', function(data)
    if not hasJob then return end
    openLicenseMenu(data)
end)

AddEventHandler('rom_cia:purchaseArmoury', function(data)
    if not hasJob then return end
    local data = data
    data.quantity = 1
    ...
    local canPurchase = lib.callback.await('rom_cia:canPurchase', 100, data)
    if canPurchase then
        TriggerEvent('rom_cia:notify', Strings.success, Strings.successful_purchase_desc, 'success')
    else
        TriggerEvent('rom_cia:notify', Strings.lacking_funds, Strings.lacking_funds_desc, 'error')
    end
end)

AddEventHandler('rom_cia:checkId', function(targetId)
    if not hasJob then return end
...
AddEventHandler('rom_cia:revokeLicense', function(data)
    TriggerServerEvent('esx_license:removeLicense', data.targetId, data.license)
    TriggerEvent('rom_cia:notify', Strings.license_revoked, Strings.license_revoked_desc, 'success')
    Wait(420) -- lul
    checkPlayerId(data.targetId)
end)

AddEventHandler('rom_cia:manageId', function(data)
    manageId(data)
end)
...
CreateThread(function()
    ...
    for i=1, #Config.ciaJobs do
        if Config.ciaJobs[i] == ESX.PlayerData.job.name then
        ...
        if v.bossMenu.enabled then
            if v.bossMenu?.target?.enabled then
                local data = {
                    ...
                    options = {
                        {
                            event = 'rom_cia:openBossMenu',
                            icon = 'fa-solid fa-suitcase-medical',
                            label = v.bossMenu.target.label
                        }
                    }
                }
                TriggerEvent('rom_cia:addTarget', data)
            else
                CreateThread(function()
                    local textUI
                    while true do
...
if Config.handcuff.hotkey then
    RegisterCommand('cuffPlayer', function()
        TriggerEvent('rom_cia:handcuffPlayer')
    end)
...
AddEventHandler('rom_cia:pdJobMenu', function()
    openJobMenu()
end)
