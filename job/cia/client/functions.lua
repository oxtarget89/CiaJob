...
escortPlayer = function(targetId)
    local targetCuffed = lib.callback.await('rom_cia:isCuffed', 100, targetId)
    if targetCuffed then
        TriggerServerEvent('rom_cia:escortPlayer', targetId)
    else
        TriggerEvent('rom_cia:notify', Strings.not_restrained, Strings.not_restrained_desc, 'error')
    end
end
...
handcuffPlayer = function(targetId)
    ...
    if deathCheck(targetId) then
        TriggerEvent('rom_cia:notify', Strings.unconcious, Strings.unconcious_desc, 'error')
    else
        TriggerServerEvent('rom_cia:handcuffPlayer', targetId)
    end
end
...
local startCuffTimer = function()
    ...
    cuffTimer.timer = ESX.SetTimeout(Config.handcuff.timer,function()
        TriggerEvent('rom_cia:uncuff')
    end)
end

handcuffed = function()
    isCuffed = true
    TriggerServerEvent('rom_cia:setCuff', true)
...
uncuffed = function()
    ...
    isCuffed = false
    TriggerServerEvent('rom_cia:setCuff', false)
...
checkPlayerId = function(targetId)
    ESX.TriggerServerCallback('rom_cia:checkPlayerId', function(data) 
        local Options = {
            {
                title = Strings.go_back,
                description = '',
...
vehicleInfoMenu = function(vehicle)
    if not DoesEntityExist(vehicle) then
        ...
        local ownerData = lib.callback.await('rom_cia:getVehicleOwner', 100, data.plate)
        local Options = {
            {
                title = Strings.go_back,
                description = '',
                arrow = false,
                event = 'rom_cia:vehicleInteractions',
            },
            {
                title = Strings.plate,
    ...
    if not DoesEntityExist(vehicle) then
        ...
        if dist < 2.5 then
            ...
            if lib.progressCircle({
                ...
                TriggerEvent('rom_cia:notify', Strings.lockpicked, Strings.lockpicked_desc, 'success')
            else
                TriggerEvent('rom_cia:notify', Strings.cancelled, Strings.cancelled_desc, 'error')
            end
        else
    ...
    if not DoesEntityExist(vehicle) then
        ...
        if dist < 2.5 then
            ...
            if driver == 0 then
                ...
                TriggerEvent('rom_cia:notify', Strings.driver_in_car, Strings.driver_in_car_desc, 'error')
            end
        else
            TriggerEvent('rom_cia:notify', Strings.too_far, Strings.too_far_desc, 'error')
        end
...
armouryMenu = function(station)
    ...
    if allow then
        ...
        elseif not data.weapons[ESX.PlayerData.job.grade] then
            print('[rom_cia] : ARMORY NOT SET UP PROPERLY FOR GRADE: '..ESX.PlayerData.job.grade)
        else
        ...
        TriggerEvent('rom_cia:notify', Strings.no_permission, Strings.no_access_desc, 'error')
    end
end
...
openVehicleMenu = function(station)
    ...
    elseif not Config.Locations[station].vehicles.options[ESX.PlayerData.job.grade] then
        print('[rom_cia] : cia garage not set up properly for job grade: '..ESX.PlayerData.job.grade)
        return
    else
...
attemptTackle = function()
    ...
    if dist ~= -1 and dist < 2.01 and not isBusy and not IsPedInAnyVehicle(cache.ped) and not IsPedInAnyVehicle(GetPlayerPed(player)) and GetGameTimer() - lastTackle > 7 * 1000 then
        if Config.tackle.ciaOnly then
            if hasJob then
                lastTackle = GetGameTimer()
                TriggerServerEvent('rom_cia:attemptTackle', GetPlayerServerId(player))
            end
        else
            lastTackle = GetGameTimer()
            TriggerServerEvent('rom_cia:attemptTackle', GetPlayerServerId(player))
